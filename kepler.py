...
"""
This module is used to solve keplers equations. It can solve for kinetic energy, potential energy, total energy, dervivatives and the orbit pathway as well as initiating the system.
"""
...
def total_energy(z,m):
    ...
    Arguments
        z (array-like)
            vector containing positon and velocity
        m (scalar)
            total mass in normalized units
    """
...
def derivs(t,z,m):
    ...
    Arguments
        ...
        z (array-like)
            vector containing postion and velocity
        m (scalar) 
            total mass in normalized units
    Returns
...
def integrate_orbit(z0,m,tend,h,method='RK4'):
    ...
    Arguments:
        z0 (array-like)
            Vector containing inital postion and velocity
...
def set_initial_conditions(a, m, e):
    ...
    eps0 = -m/(2*a)
    # period of motion
    Tperiod = (np.pi/np.sqrt(2))*m*np.abs(eps0)**(-3/2)
